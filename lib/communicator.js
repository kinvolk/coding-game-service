...
const ExternalEffects = new Lang.Class({
    ...
    changeGSettingsValue: function(schema, key, value) {
        _getSettingsForSchema(schema).set_value(key, value);
    },

    fetchGSettingsValue: function(schema, key, variant) {
        // Variant is unused here, but we use it in the tests
        // to construct a placeholder
        return _getSettingsForSchema(schema).get_value(key).deep_unpack();
    },
