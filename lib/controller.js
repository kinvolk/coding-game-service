...
const CodingGameController = new Lang.Class({
    ...
    _dispatchChatAttachmentEvent: function(event, callback) {
        ...
        // If the provided attachmentPath is actually a desktop app id
        // then locate the full path for it.
        let appInfo = Gio.DesktopAppInfo.new(attachmentPath);
        if (appInfo) {
            attachmentPath = appInfo.get_filename();
        }

        event.data.attachment.path = resolvePath(attachmentPath,
    ...
    _resetGame: function() {
        ...
        // Tell the chatbox to clear its logs too
        this._chatController.reset();
    ...
    _changeSettingEvent: function(event, callback) {
        try {
            // We need to pass variant_type here so that the tests know
            // what to return here.
            let value = resolveGSettingsValue(event.data.value,
                                              this._effectsManager.fetchGSettingsValue(event.data.schema,
                                                                                       event.data.key,
                                                                                       event.data.variant_type));
            this._effectsManager.changeGSettingsValue(event.data.schema,
                                                      event.data.key,
                                                      new GLib.Variant(event.data.variant_type, value));
        } catch (e) {
            throw new Error('Error ocrurred when changing setting ' + String(e));
        }
