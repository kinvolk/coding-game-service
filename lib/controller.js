...
// realistic.
function determineWaitTimeForEvent(event) {
    switch (event.type) {
        case 'chat-actor':
            ...
            // too long.
            return Math.min(event.data.message.length * 120, 1500);
        case 'chat-actor-attachment':
            // Attachment. Fixed 1.5 second delay + character length
            // of description
            return 1500 + Math.min(event.data.attachment.desc.length * 60,
                                   500);
        case 'input-user':
...
const CodingGameController = new Lang.Class({
    ...
    receiveChatResponse: function(id, contents, response) {
        ...
        this._progressivelyDispatch(toDispatch);
    },
    ...
    _startMission: function(name) {
        ...
        this._service.setGameManagerState(missionState.gameManagerState);
        let toDispatch = missionState.descriptor.start_events.map(Lang.bind(this, function(start_event) {
            let event = findInArray(this._descriptors.events, function(e) {
    ...
    _completeWaitForEventIn: function(event, timeout) {
        this._effectsManager.performEventIn(timeout, Lang.bind(this, function() {
            this._dispatch({
    ...
    // Dispatch each event in events, waiting a calculated number of
    // seconds between them. This spaces events out so that
    // chat-interactions seem more realistic.
    //
    // Note that this isn't a good long-term solution - really, the timeline
    // should be using wait-for events, but that would require adding a lot
    // of wait-for events.
    _progressivelyDispatch: function(events) {
         // Nothing to dispatch, return early
         if (!events.length)
             return;

         let dispatcher = Lang.bind(this, function() {
             let event = events.shift();

             this._dispatch(event);
             if (events.length) {
                // Determine a sensible timeout by reference to the
                // input type. Again, a hack that seems to work quite
                // well.
                let timeout = determineWaitTimeForEvent(events[0]);
                this._effectsManager.performEventIn(timeout,
                                                    dispatcher);
             }
         });

         this._effectsManager.performEventIn(determineWaitTimeForEvent(events[0]),
                                             dispatcher);
    }
});
