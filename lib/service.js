...
const DBUSService = new Lang.Class({
    ...
    setGameManagerState: function(properties) {
        ...
        this.earned_artifacts = properties.earnedArtifacts.map(Lang.bind(this, function(a) {
            return JSON.stringify(artifactForStage(a, this.current_mission_stage_num));
        }));
    },
    ...
    completeTaskWithPoints: function(points, artifact) {
        ...
        this.earned_artifacts = this.earned_artifacts.map(function(a) {
            return JSON.parse(a);
        }).concat([artifactForStage(artifact, this.current_mission_stage_num)]).map(function(a) {
