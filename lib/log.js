...
const CodingGameServiceLog = new Lang.Class({
    ...
    chatLogForActor: function(actor) {
        return this._eventLog.filter(function(e) {
            return (e.type === 'chat-actor' ||
                    ...
                    e.type === 'chat-actor-desktop-attachment' ||
                    e.type === 'chat-user' ||
    ...
    allUniqueArtifactEvents: function() {
        let eventsToEntries = {};
        ...
        return Object.keys(eventsToEntries).map(function(key) {
            return eventsToEntries[key];
        });
    },
    ...
    activeTimeouts: function() {
        ...
        this._eventLog.forEach(function(e) {
            if (e.type === 'wait-for') {
    ...
    getChangedSettings: function() {
        ...
        this._eventLog.forEach(function(e) {
            if (e.type === 'change-setting')
                descriptions.add(e.data.schema + ':' + e.data.key);
        });

        return [...descriptions].map(function(d) {
            let [schema, key] = d.split(':');
            return {
                schema: schema,
                key: key
            }
    ...
    reversedAppGridModifications: function() {
        // Go through the event log in reverse and yield modify-app-grid
        // events that should effectively reverse the current state
        return this._eventLog.filter(function(e) {
            return e.type === 'modify-app-grid';
        }).reverse();
    },

    reversedCopyFileEvents: function() {
      // Go through the event log in reverse and yield copy-file
      // events that should effectively reverse the current state
      return this._eventLog.filter(function(e) {
        return e.type === 'copy-file';
      }).reverse();
    },
});
